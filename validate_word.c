...
int check_word(struct node_t* tree, char* word)
{
	for (int j = 0; j < strlen(word); j++)
	{
		int is_there_letter = 0;
		for (int i = 0; i < tree->num_children; i++)
			if (tree->children[i]->key == *word)
			{
				is_there_letter = 1; break;
			}
		if (!is_there_letter)
			return 0;
		word++;
...
int validate_word(struct node_t* tree, char* word)
{
	...
	//if (!strcmp(tree->key, word))
	//{
	//	//if (check_word(tree, word++))
	//	return 1;
	//}
	//else
	//	for (int i = 0; i < tree->num_children; i++)
	//		validate_word(tree->children[i], word, res);
